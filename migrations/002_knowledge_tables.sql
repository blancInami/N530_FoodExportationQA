...
COMMENT ON TABLE  public."知識文獻節點檔"                IS '文獻中的各節點（條號 / 章節 / Q&A）';
COMMENT ON COLUMN public."知識文獻節點檔"."節點標題路徑" IS '完整階層路徑，如 "第二章 > 第十五條" 或 "Q1: 申請資格為何"';
...
COMMENT ON TABLE  public."知識文獻切塊檔"            IS '節點內容的滑動視窗切塊 + 1024-dim 向量（由 tools/ingest_agent.py 生成）';
COMMENT ON COLUMN public."知識文獻切塊檔"."內容向量" IS 'intfloat/multilingual-e5-large-instruct 1024-dim cosine vector';

-- ── Step 6: 建立索引 ─────────────────────────────────────────────────────────
CREATE INDEX IF NOT EXISTS idx_知識文獻節點檔_主檔
